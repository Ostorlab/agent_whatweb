...
VULNZ_DESCRIPTION = 'Lists web technologies including content management systems(CMS), blogging platforms, statistic/analytics packages, JavaScript  libraries, web servers, embedded devices, version numbers, email addresses, account IDs, web framework modules, SQL errors, and more.'
        ...
        if len(versions) > 0:
            for version in versions:
                ...
                self.report_vulnerability(
                    ...
                    technical_detail=f'Found library `{library_name}`, version `{str(version)}`, of type `{fingerprint_type}` in domain `{domain_name}`',
